...
public class WebSite {
    ...
    private boolean isAdmin(WebAccountRepository.AccountData account) {
        if (account == null) return false;
        // проверяем права antigrief.admin
        List<String> nicks = plugin.getPlayerNickRepository().getNicksByAccount(account.id);
        for (String nick : nicks) {
            org.bukkit.OfflinePlayer op = plugin.getServer().getOfflinePlayer(nick);
            if (op.isOp() || op.getPlayer() != null && op.getPlayer().hasPermission("antigrief.admin")) {
                return true;
            }
        }
        return false;
    ...
    private String htmlPage(String title, String bodyContent, String extraScripts) {
        return "<!DOCTYPE html><html lang='ru'><head>"
            ...
            + "<script type='module'>import FP from '/fp.js'; window.FingerprintJS=FP; window.dispatchEvent(new Event('fpReady'));</script>"  // fpjs v5 es-модуль
            + FINGERPRINT_JS  // функции — ждёт fpReady
            + "</head><body><div class='container'>"
            + bodyContent
            + "</div>"
            + "<div class='footer'><b>VNLLA.RU</b> &copy; 2026 | by Calladius</div>"
            + FP_BAN_CHECK  // проверка бан-маркера
            + extraScripts
            + "</body></html>";
    }
